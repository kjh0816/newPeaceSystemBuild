...
class VendorService(
    ...
    fun modifyFuneralIntoPoltraitId(funeralId: Int, portraitId: Int): ResultData {
        val funeral = getFuneralById(funeralId) ?: return ResultData.from("F-1", "올바르지 않은 접근입니다.")

        if(funeral.portraitId != 0){
            return ResultData.from("F-3", "이미 영정액자를 신청하였습니다.")
        }
        ...
        vendorRepository.insertIntoOrder(client.id, rq.getLoginedMember()!!.id, roleCategoryId, portraitId)
        ...
        // location으로 조회했을 때, 해당 지역에 한 명도 없는 경우에 대한 예외처리
        if(vendors.isEmpty()){
            return ResultData.from("F-2", "${client.location}에 등록된 영정액자 업체가 없습니다.")
        }
        ...
        // funeral 테이블에 portraitId 를 업데이트 한다.
        vendorRepository.modifyFuneralIntoPortraitId(funeralId, portraitId)
        // 연결된 물품 공급업자에게 주문 정보를 주기 위해 orderId를 성공 시, 같이 return
