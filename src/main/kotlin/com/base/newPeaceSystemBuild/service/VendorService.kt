...
class VendorService(
    ...
    fun modifyFuneralIntoFlowerTributeId(funeralId: Int, flowerTributeId: Int, bunchCnt: Int): Any {
        val funeral = getFuneralById(funeralId) ?: return ResultData.from("F-1", "올바르지 않은 접근입니다.")
        ...
        // 포장 헌화를 선택했는지 포장 안된헌화를 선택했는지
        var packing: Boolean = false

        if(flowerTributeId == 1){
            packing = false
        }
        else if(flowerTributeId == 2){
            packing = true
        }

        vendorRepository.insertIntoFlowerTributeOrder(orderId, bunchCnt, packing)
        ...
        // funeral 테이블에 flowerTributeId 를 업데이트 한다.
